...
public class TestCompartilhamentoCardLink2 {
	...
	public void Go () throws InterruptedException {
		...
		if (Parametros.statusBug.equalsIgnoreCase("Qualidade")) {
			...
			try {
				...
				Retestar.GoCompartilhamentoCard();
				
				Jira Aprovado = new Jira();
				Aprovado.UrlJira(Parametros.UrlJira);
				Aprovado.PesquisarTask(TestCompartilhamentoCardLink.Resumo);
				Aprovado.AcessarBug();
				Aprovado.FinalizarBug();
				Aprovado.ComentarioAprovado();
				Aprovado.FinalizarBugArquivoTXT(TestCompartilhamentoCardLink.BugArquivoTXT);
				Aprovado.Close();
				Aprovado.UrlJira(Parametros.UrlJira);
				
				System.out.println("Caso de Teste: ( TestCompartilhamentoCardLink )" + "\n" + "Retestado e aprovado!" + "\n");
							
			} catch (Throwable e) {

				Jira Reprovado = new Jira();
				Reprovado.UrlJira(Parametros.UrlJira);
				Reprovado.PesquisarTask(TestCompartilhamentoCardLink.Resumo);
				...
				Assert.fail("Caso de Teste: ( TestCompartilhamentoCardLink )" + "\n" + "Retestado e reprovado!" + "\n");
