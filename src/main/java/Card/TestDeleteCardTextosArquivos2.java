...
public class TestDeleteCardTextosArquivos2 {
	...
	public void Go () throws InterruptedException {
		...
		if (Parametros.statusBug.equalsIgnoreCase("Qualidade")) {
			...
			try {
				...
				Retestar.GoDeleteCardTextosArquivos();
				
				Jira Aprovado = new Jira ();
				Aprovado.UrlJira(Parametros.UrlJira);
				Aprovado.PesquisarTask(TestDeleteCardTextosArquivos.Resumo);
				Aprovado.AcessarBug();
				Aprovado.FinalizarBug();
				Aprovado.ComentarioAprovado();
				Aprovado.FinalizarBugArquivoTXT(TestDeleteCardTextosArquivos.BugArquivoTXT);
				Aprovado.Close();
				Aprovado.UrlJira(Parametros.UrlJira);
			
				System.out.println("Caso de Teste: ( TestDeleteCardTextosArquivos )" + "\n" + "Retestado e aprovado!" + "\n");
						
			} catch (Throwable e) {

				Jira Reprovado = new Jira();
				Reprovado.UrlJira(Parametros.UrlJira);
				Reprovado.PesquisarTask(TestDeleteCardTextosArquivos.Resumo);
				...
				Assert.fail("Caso de Teste: ( TestDeleteCardTextosArquivos )" + "\n" + "Retestado e reprovado!" + "\n");
